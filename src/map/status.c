...
// ----- STATS CALCULATION -----
	...
	if((skill=pc_checkskill(sd,SA_DRAGONOLOGY))>0)
		base_status->int_ += (skill+3)/1; // +1 INT / 2 lv
...
// ----- FLEE CALCULATION -----
	...
	if((skill=pc_checkskill(sd,TF_MISS))>0)
		base_status->flee += skill*(sd->class_&JOBL_2 && (sd->class_&MAPID_BASEMASK) == MAPID_THIEF? 10 : 8);
	if((skill=pc_checkskill(sd,MO_DODGE))>0)
		base_status->flee += (skill*20)>>1;
	if (pc_checkskill(sd, SU_POWEROFLIFE) > 0)
		base_status->flee += 20;
...
void status_calc_regen(struct block_list *bl, struct status_data *status, struct regen_data *regen)
{
	...
	if( sd ) {
		...
		if( (skill=pc_checkskill(sd,SM_RECOVERY)) > 0 )
			val += skill*50 + skill*status->max_hp/500;
		sregen->hp = cap_value(val, 0, SHRT_MAX);

		val = 0;
		if( (skill=pc_checkskill(sd,MG_SRECOVERY)) > 0 )
			val += skill*30 + skill*status->max_sp/500;
		...
		if( (skill=pc_checkskill(sd,MO_SPIRITSRECOVERY)) > 0 )
			val += skill*40 + skill*status->max_hp/500;

		if( (skill=pc_checkskill(sd,TK_HPTIME)) > 0 && sd->state.rest )
			val += skill*300 + skill*status->max_hp/500;
		sregen->hp = cap_value(val, 0, SHRT_MAX);
